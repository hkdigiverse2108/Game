class Snake{
	constructor(ctx, name, id){		
		...
		// initialize player in world coordinates (center of configured world)
		this.pos = new Point(game.world.x + game.WORLD_SIZE.x/2, game.world.y + game.WORLD_SIZE.y/2);
	...
	respawn(){
		...
		// pick a random safe spawn location
		this.pos = new Point(
			ut.random(Math.floor(game.world.x + 100), Math.floor(game.world.x + game.WORLD_SIZE.x - 100)),
			ut.random(Math.floor(game.world.y + 100), Math.floor(game.world.y + game.WORLD_SIZE.y - 100))
		);
		this.angle = ut.random(0, Math.PI);
