...
MainGame.LoadingUI = {
    ...
    setProgress: function(progress) {
        var ui = this.ensure();
        var value = Math.max(0, Math.min(1, progress || 0));
        var percent = Math.round(value * 100);
        if (ui.fill) ui.fill.style.width = percent + "%";
        if (ui.percent) ui.percent.textContent = percent + "%";
        if (ui.root) ui.root.setAttribute("aria-valuenow", String(percent))
    },
    finish: function() {
        var ui = this.ensure();
        if (ui.root) {
...
class Preloader extends Phaser.Scene {
    ...
    preload() {
        ...
        MainGame.LoadingUI.ensure();
        MainGame.LoadingUI.setStatus("Loading assets");
        MainGame.LoadingUI.setProgress(0);
    ...
    updateLogoCrop(loadProgress) {
        MainGame.LoadingUI.setProgress(loadProgress);
        if (loadProgress >= 0.9) {
            MainGame.LoadingUI.setStatus("Almost ready");
        } else if (loadProgress >= 0.5) {
            MainGame.LoadingUI.setStatus("Loading world");
        } else {
            MainGame.LoadingUI.setStatus("Loading assets");
        }
    ...
    onLoadComplete() {
        ...
        MainGame.LoadingUI.setProgress(1);
        MainGame.LoadingUI.setStatus("Starting game");
        if (MainGame.isAPI) MainGame.API_POKI.gameLoadingFinished()
        window.setTimeout(function() {
            MainGame.LoadingUI.finish()
        }, 140)
    }
    initResize() {
